"""Modelo de configuração: lê/escreve theme.conf e o drop-in do SDDM."""
...
# ---------------------------------------------------------------------------
# Especificação das chaves do tema (seção [General] do theme.conf).
# (default, tipo) — tipo em {"str", "bool", "int", "float"}
# ---------------------------------------------------------------------------
THEME_SPEC: dict[str, tuple[object, str]] = {
    ...
    "dateFormat": ("dddd, d 'de' MMMM", "str"),
    "dateLocale": ("pt_BR", "str"),

    # estilos de widget
    "inputStyle": ("underline", "str"),    # underline | box | pill | segmented
    "buttonStyle": ("outline", "str"),     # outline | fill | pill   (LOGIN)
    "controlStyle": ("text", "str"),       # text | outline | pill   (sessão/energia)
...
# ---------------------------------------------------------------------------
# Especificação do drop-in do SDDM (/etc/sddm.conf.d/10-maia.conf).
# secção -> {chave: default}
# ---------------------------------------------------------------------------
...
class SddmConfig:
    ...
    def _load_theme(self, conf_path: Path) -> None:
        ...
        if parser.has_option("General", "layoutJson"):
            ...
            if raw and not raw.lstrip().startswith("{"):
                try:  # base64 (o SDDM tira as aspas de valores do theme.conf)
    ...
    def theme_conf_text(self) -> str:
        lines = [
            "[General]",
            "# Gerado por sddm-forge. Editado pela GUI.",
            "",
        ]
        for key, (_default, typ) in THEME_SPEC.items():
            lines.append(f"{key}={_fmt(self.theme.get(key, _default), typ)}")
        # layout embutido em base64 — o SDDM remove as aspas de valores do
        # theme.conf, então JSON puro não sobrevive. base64 só tem [A-Za-z0-9+/=].
        # O greeter e a prévia leem via `config.layoutJson`; layout.json ao lado
        # é só p/ inspeção.
    ...
    def dropin_text(self) -> str:
        out: list[str] = ["# Gerado por sddm-forge — não editar à mão.", ""]
        for sec, keys in SDDM_SPEC.items():
            body: list[str] = []
            for key in keys:
    ...
    def _find(self, node_id: str, children: list[dict] | None = None,
        ...
        """Retorna (node, siblings_list) ou (None, None)."""
    ...
    def add_panel(
        ...
        *,
        ...
        n = len(self.panels()) + 1
        panel = make_panel(name or f"Painel {n}", orientation=orientation)
        if orientation == "row":
            panel["align"] = "center"
        ...
        if parent_id is None:
            ...
            if parent is None or parent.get("type") != "panel":
                return panel
            if name is None:
                panel["name"] = "Divisão" if orientation == "row" else "Sub-painel"
    ...
    def reorder_node(self, node_id: str, target_id: str, after: bool) -> None:
        """Move `node_id` p/ imediatamente antes/depois de `target_id`
        (pode trocar de painel). Ignora se alvo == nó ou alvo dentro do nó."""
        ...
        if node.get("type") == "panel" and self._contains(node, target_id):
            return
        # folha não pode virar irmã de painel na raiz — cai dentro do painel
